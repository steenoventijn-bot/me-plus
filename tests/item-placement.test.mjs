...
test('item preview requires confirmation, rejects water, cancels safely and sends only one save',async()=>{
 ...
 globalThis.document={hidden:false,addEventListener(){},createElement(){return {getContext(){return {createLinearGradient(){return {addColorStop(){}}},createRadialGradient(){return {addColorStop(){}}},fillRect(){}}}}}};
 ...
 for(const name of ['models','placement','life','adventure','environment'])source=source.replace(`'./${name}.js'`,`'${pathToFileURL(process.cwd()+'/world/'+name+'.js')}'`);
