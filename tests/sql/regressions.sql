...
begin
 ...
 result:=public.me_world_save_position(own,null,'bench_wood',9.23,13.47,90);placed_id:=(result->>'id')::uuid;
 ...
 begin
  perform public.me_world_save_position(other,placed_id,null,8,13,0);
  raise exception 'Cross-account move accepted' using errcode='ZX001';
 exception when sqlstate 'P0001' then null;end;
 result:=public.me_world_save_position(own,placed_id,null,8.12,13.81,270);
