...
export function terrain(){
 const n=128,rings=[0,.12,.24,.36,.48,.60,.72,.81,.87,.90,.925,.95,.97,.99,1.015,1.045,1.075];
 ...
 for(let i=0;i<38;i++){const a=i/38*Math.PI*2+.04*rng();if(i%7===3||i%7===4)continue;const r=coastRadius(a)*(1.003+rng()*.014);rock.ball(['#b6b5a5','#cfc7b2','#a2aca5'][i%3],Math.cos(a)*8.7*r,-.10,Math.sin(a)*8.15*r,.38+rng()*.24,.37+rng()*.22,.28+rng()*.17)}
...
export function ocean(){
 ...
 void main(){
  ...
  vec3 eye=normalize(cameraPosition-vec3(p.x,-.48,p.y));vec3 halfLight=normalize(eye+normalize(vec3(-.4,.85,.3)));
  ...
  float swash=1.05+.008*sin(time*.65+n*4.);
 ...
 const geo=new T.PlaneGeometry(120,120);geo.rotateX(-Math.PI/2);const mesh=new T.Mesh(geo,material);mesh.position.y=-.48;mesh.name='ocean-shallows-reflections-foam';return mesh;
}
