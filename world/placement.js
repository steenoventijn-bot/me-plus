...
export function footprint(key,rotation=0) {
  key = ({flowers_white:'flowers_pink',tropical_bush:'bush_round',pond_lily:'pond_small',pool_small:'pond_garden',stream_stones:'pond_garden',waterfall_garden:'pond_garden',fountain_gold:'fountain',fence_white:'fence_wood',bookshelf_small:'knowledge_stack',knowledge_library:'knowledge_house',social_house:'knowledge_house',training_house:'growth_zone',streak_house60:'knowledge_house',knowledge_stand:'growth_zone',trophy_bronze:'trophy_battle',trophy_silver:'trophy_battle',trophy_gold:'trophy_battle',trophy_champion:'trophy_battle',streak_palm7:'palm_tree',streak_beach14:'bench_wood',streak_lights30:'lamp_glow',streak_legend100:'trophy_battle',beach_chair:'bench_wood',parasols:'bush_round',activity_garden:'social_flag',sign_meplus:'social_flag'})[key] || key;
  let [w,h] = ({house_main:[3.45,3.2],pond_garden:[3.1,2.5],dock_wood:[1.5,2.6],tree_oak:[.62,.62],tree_streak7:[.7,.7],palm_tree:[.58,.58],flowers_pink:[.85,.62],bush_round:[.85,.78],bench_wood:[1.35,.55],lamp_glow:[.32,.32],pond_small:[1.9,1.45],campfire:[.95,.95],fountain:[1.4,1.4],knowledge_house:[2,1.7],growth_zone:[1.8,1.6],trophy_battle:[.65,.65],social_flag:[.32,.32],knowledge_stack:[.65,.5],golden_duck:[.65,.48],fence_wood:[1.3,.23],bridge_wood:[1.8,.9]})[key]||[.7,.7];
...
export function placementResult({key,x,y,rotation=0,id=null},items=[]) {
  const shape={x,y,...footprint(key,rotation)};
  if(![0,90,180,270].includes(rotation)||!validLand(x,y,shape,key))return {valid:false,reason:'Plaats het hele object op het gras.'};
  if(items.some(it=>it.id!==id&&overlaps(shape,{...itemPosition(it),...footprint(it.item_key,Number(it.rotation||0))})))return {valid:false,reason:'Er staat al een object op deze plek.'};
