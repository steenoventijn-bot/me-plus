...
export function createWorld(host,{onSelect=()=>{},onDrop=()=>{},onHint=()=>{},onError=()=>{},onCameraChange=()=>{}}={}){
 ...
 function preview(x,y){if(!drag||!data)return null;const p=ground(x,y);if(!p){ring.visible=false;return null}const q={key:drag.key,id:drag.id,rotation:drag.rotation,...p},result=placementResult(q,data.items);showGhost(drag.key);ghost.position.set(p.x-9.5,groundY+.1,p.y-9.5);ghost.rotation.y=-drag.rotation*Math.PI/180;ring.visible=true;ring.material.color.set(result.valid?'#2fe797':'#ff4e52');const f=footprint(drag.key,drag.rotation);ring.scale.set(f.w,f.h,1);ring.position.set(p.x-9.5,groundY+.04,p.y-9.5);onHint(result.reason,result.valid);return{...q,...result};}
