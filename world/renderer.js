...
export function createWorld(host,{onSelect=()=>{},onDrop=()=>{},onHint=()=>{},onError=()=>{},onCameraChange=()=>{}}={}){
 ...
 function render(now){frame=0;if(disposed||paused||document.hidden||!canvas.isConnected){last=0;return}frame=requestAnimationFrame(render);const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;if(last&&now-last<(reduced?100:1000/30))return;const dt=last?now-last:33;last=now;water.material.uniforms.time.value=reduced?0:now/1000;renderer.render(scene,camera);frames++;ms+=dt;if(frames===90){const fps=90000/ms;canvas.dataset.fps=fps.toFixed(1);canvas.dataset.drawCalls=renderer.info.render.calls;canvas.dataset.triangles=renderer.info.render.triangles;if(fps<23&&renderer.getPixelRatio()>1){renderer.setPixelRatio(1);resize();slow++}frames=0;ms=0}}
 ...
 renderer.shadowMap.needsUpdate=true;updateContactShadows(next.items);updatePaths();if(changedOwner){awaitingPlacement=false;lastPlacement=null;zoom=1;pan.set(0,0);updateCamera();clearDrag()} }
 ...
 dispose(){disposed=true;cancelAnimationFrame(frame);abort.abort();observer.disconnect();clearHold();clearDrag();scene.traverse(o=>o.geometry?.dispose());scene.background.dispose();water.material.dispose();ring.material.dispose();contactShadows?.dispose();contactGeometry.dispose();contactMaterial.dispose();renderer.dispose();canvas.remove()}};
}
