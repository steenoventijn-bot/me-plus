...
export function createWorld(host,{onSelect=()=>{},onDrop=()=>{},onHint=()=>{},onError=()=>{},onCameraChange=()=>{}}={}){
 ...
 const sun=lighting(scene);const hour=new Date().getHours();if(hour>=19||hour<7){sun.color.set('#ffdcaa');sun.intensity=2.5}scene.add(terrain(),grass());const life=createLife(scene);const water=ocean();scene.add(water);let paths=new T.Group();scene.add(paths);
 ...
 function render(now){frame=0;if(disposed||paused||document.hidden||!canvas.isConnected){last=0;return}frame=requestAnimationFrame(render);const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;if(last&&now-last<(reduced?100:1000/30))return;const dt=last?now-last:33;last=now;water.material.uniforms.time.value=reduced?0:now/1000;life.tick(now,Math.min(dt/1000,.1),reduced,edit);renderer.render(scene,camera);if(frames%6===0)onCameraChange();frames++;ms+=dt;if(frames===90){const fps=90000/ms;canvas.dataset.fps=fps.toFixed(1);canvas.dataset.drawCalls=renderer.info.render.calls;canvas.dataset.triangles=renderer.info.render.triangles;if(fps<23&&renderer.getPixelRatio()>1){renderer.setPixelRatio(1);resize();slow++}frames=0;ms=0}}
 ...
 return {canvas,update,people:life.people,project:life.project,point:ground,select(id){selected=id},setEdit(value){edit=value;if(!edit){awaitingPlacement=false;clearDrag()}},setZoom(delta){zoom=T.MathUtils.clamp(zoom+delta,.75,2.25);updateCamera()},reset(){zoom=1;pan.set(0,0);updateCamera()},pause(v){paused=v||!!document.querySelector?.('.w16-album,.w15-browser,.w14-house-sheet');last=0;cancelAnimationFrame(frame);frame=0;if(!paused&&!disposed&&!document.hidden)frame=requestAnimationFrame(render)},mount(node){host=node;host.appendChild(canvas);observer.disconnect();observer.observe(host);resize()},project(x,y,height=.5){const p=new T.Vector3(x-9.5,height,y-9.5).project(camera);return{x:(p.x+1)/2*canvas.clientWidth,y:(1-p.y)/2*canvas.clientHeight}},
 beginPlacement(key){
